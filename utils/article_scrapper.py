...
def pressgazette(url) -> dict:
    ...
    """
    logging.basicConfig(filename='log_file.log' ,level=logging.DEBUG, format = " %(levelname)s - %(asctime)s - %(messages)s")
    paragraph_content = []
    ...
    try:    
        ...
        logging.info("data scraped successfully for pressgazette")
        
        return headline , paragraph_content ,url
        # return {'Headline': headline , 'Text': paragraph_content,'Article-Url': url}
    
    except Exception as  e:
        logging.error(f"Unable to fetch data for pressgazette {e}")
        

    
def thehindubusinessline(url) -> any:
    """
    Takes url as a parameter and extracts news headline and text content
    ...
    try:    
        ...
        for paragraph in paragraphs:
            paragraph_content.append(paragraph.text.replace("\n", ""))  
        logging.info("data scraped successfully for thehindubusinessline")
        
        return headline , paragraph_content , url
        # return {'Headline': headline ,'Text': paragraph_content,'Article-Url':url }
    
    except Exception as e:
        logging.error(f"Unable to fetch data for thehindubusinessline: {e}")